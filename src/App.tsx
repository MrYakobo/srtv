...
const LS_KEY = "flashy-video-state";
...
export const App: React.FC = () => {
  ...
  const [presetIdx, setPresetIdx] = useState(saved.presetIdx ?? 0);
  const [customW, setCustomW] = useState(saved.customW ?? 1080);
  const [customH, setCustomH] = useState(saved.customH ?? 1920);
  ...
  // Persist
  useEffect(() => {
    const state: SavedState = { srtText, beatSensitivity, presetIdx, customW, customH, panelWidth, srtTab, audioFileName: audioFile?.name };
    localStorage.setItem(LS_KEY, JSON.stringify(state));
  }, [srtText, beatSensitivity, presetIdx, customW, customH, panelWidth, srtTab, audioFile]);
  ...
  const handleAudio = useCallback(async (file: File) => {
    setError(""); setAudioFile(file); setAnalyzing(true); setReady(false);
    try {
      setAudioUrl(URL.createObjectURL(file));
      const result = await analyzeAudio(await file.arrayBuffer(), FPS);
      setRawEnergies(result.energies); setDuration(result.duration); setReady(true);
  ...
  const inputProps: FlashyVideoProps = { subtitles, audioSrc: audioUrl, energies: rawEnergies, beats };
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      const { getBlob } = await renderMediaOnWeb({
        ...
        composition: { component: FlashyVideo, durationInFrames: Math.max(totalFrames, 30), fps: FPS, width: compWidth, height: compHeight, id: "FlashyLyricVideo", defaultProps: inputProps },
        inputProps, container: "mp4", videoCodec: "h264", videoBitrate: "high",
        onProgress: ({ progress }) => { setExportProgress(Math.round(progress * 100)); },
      });
      const blob = await getBlob(); const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = "flashy-video.mp4"; a.click(); URL.revokeObjectURL(url);
    } catch (err) { console.error("Export failed:", err); setExportError(err instanceof Error ? err.message : "Export failed"); }
    finally { setExporting(false); }
  }, [totalFrames, compWidth, compHeight, inputProps]);

  /* ── Layout: full-height 2-column with cues on left ── */
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#0a0a0a", color: "#fff", overflow: "hidden" }}>
      ...
      <header style={{ padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <div style={{ width: 26, height: 26, borderRadius: 6, background: "linear-gradient(135deg,#ff006e,#8338ec)", display: "grid", placeItems: "center", fontSize: 13 }}>⚡</div>
        <h1 style={{ fontSize: 15, fontWeight: 700 }}>Flashy Lyric Video Generator</h1>
      </header>

      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        {/* ═══ Left: Cue editor (full height) ═══ */}
        <div style={{ flex: `0 0 ${panelWidth}px`, display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,0.08)", minHeight: 0 }}>
          ...
          {/* Cue content — fills remaining height */}
        ...
        {/* ═══ Right: Preview + controls (scrollable) ═══ */}
        <div style={{ flex: 1, minWidth: 280, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {exportError && <StatusBox color="#e63946" bg="rgba(230,57,70,0.12)">{exportError}</StatusBox>}
          ...
          <section>
            ...
            {analyzing && <StatusBox color="#a855f7" bg="rgba(131,56,236,0.12)">⏳ Analyzing…</StatusBox>}
            {ready && <StatusBox color="#06d6a0" bg="rgba(6,214,160,0.12)">✓ {duration.toFixed(1)}s · {totalFrames} frames</StatusBox>}
            {error && <StatusBox color="#e63946" bg="rgba(230,57,70,0.12)">{error}</StatusBox>}
          </section>
          ...
          <section>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <input type="range" min={0} max={1} step={0.01} value={beatSensitivity} onChange={(e) => setBeatSensitivity(Number(e.target.value))} style={{ flex: 1, accentColor: "#8338ec", cursor: "pointer" }} />
...
const Hint: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 4 }}>{children}</div>
);
const StatusBox: React.FC<{ color: string; bg: string; children: React.ReactNode }> = ({ color, bg, children }) => (
  <div style={{ marginTop: 6, padding: "6px 10px", background: bg, borderRadius: 6, fontSize: 12, color }}>{children}</div>
);
